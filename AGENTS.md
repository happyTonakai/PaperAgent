...
| `urlparse/` | arXiv URL/ID 归一化。`FetchURL(ctx)` / `FetchArxivAsMarkdown(ctx)` / `FetchArxivAsMarkdownFromTeX(ctx)` / `FetchArxivTitleCtx(ctx)` / `FetchArxivAbstractCtx(ctx)` 均接受 `context.Context` 支持调用方取消（浏览器关 tab → HTTP 请求取消）。HTML 优先 → TeX 源 → PDF。`LoadFile()` 读本地文件（`~` 展开）。 |
