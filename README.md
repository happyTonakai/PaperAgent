...
更麻烦的是 **KV 缓存失效**：如果每轮都把全部对话历史送进去，前缀一直在变，LLM 提供商的 KV 缓存完全用不上。每一轮都需要从头计算，Token 消耗和延迟都随轮数线性增长。
...
2. **动态上下文窗口** — 可配置的 token 输入上限（`max_input_tokens`）。对话初期，所有 QA 轮次全部保留。当累计 token 接近上限时，**硬截断**到可配置的最小轮数（`min_recent_rounds`），并以此为新「锚点」重新开始累计。
